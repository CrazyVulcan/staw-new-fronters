...
const assetPath=typeof window==='object'&&typeof window.remodulatedAssetPath==='function'?window.remodulatedAssetPath:path=>{const clean=String(path).replace(/^\/+/,'');return (assetBase?assetBase+'/':'')+(assetBase&&clean.startsWith(assetBase+'/')?clean.slice(assetBase.length+1):clean);};
...
function imageMarkup(c){const r=routes[key(c)];if(r?.localImage)return '<img class="card-image" loading="lazy" src="'+esc(assetUrl(r.localImage))+'" alt="'+esc(c.name+' — '+c.id)+'">';if(r&&r.width===1)return '<img class="card-image" loading="lazy" src="'+esc(assetUrl(r.face))+'" alt="'+esc(c.name)+'">';if(r)return '<div class="card-image" role="img" aria-label="'+esc(c.name)+'" style="background-image:url('+esc(assetUrl(r.face))+');background-size:'+r.width*100+'% '+r.height*100+'%;background-position:'+((r.index%r.width)/(r.width-1)*100)+'% '+(Math.floor(r.index/r.width)/(r.height-1)*100)+'%"></div>';return fallback(c);}
...
function exportDialog(){let output;try{const warnings=fleetWarnings().filter(s=>!s.startsWith('No SPUDS'));if(warnings.length)throw Error(warnings.join('\n'));output=FleetCore.ttsExport(fleet,routes);}catch(e){modal('<h2>TTS export needs attention</h2><p class="error-message">'+esc(e.message)+'</p><p>The supplied mod has a verified subset of the full Utopia catalog. The ID audit lists the remaining mappings.</p><a href="'+esc(assetPath('data/id-audit.json'))+'" download>Download mapping audit</a>');return;}
...
async function init(){
 [routes,aliases,costs]=await Promise.all(['tts-routes','aliases','card-costs'].map(f=>fetch(assetPath('data/'+f+'.json')).then(r=>{if(!r.ok)throw Error('Cannot load '+f);return r.json();})));
